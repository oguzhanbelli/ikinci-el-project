...
function ProductDetail() {
  ...
  const { getOneProduct, loading, buyProductDetail, state, setState } =
    useProduct();
  ...
  return (
    <div
      className={'w-screen h-screen   bg-[#F2F2F2] flex flex-col items-center pb-3  overflow-x-hidden overflow-y-auto'}
    >
      <ToastContainer
        ...
        <div className="flex flex-col  lg:flex-row mt-[20px] w-[355px] h-auto lg:w-[800px] lg:h-fit xl:w-[1480px] xl:h-[769px] bg-white rounded-[8px] ">
          <div className="flex flex-col z-50 h-f  lg:items-start lg:flex-row ">
            ...
            <div className="lg:mt-[30px] lg:ml-[60px] ml-[10px] ">
              ...
              {status >= 0 ? (
                <div className="mt-[10px] flex flex-row lg:hidden ">
                  <p className="text-[#525252]  text-[1.25em] lg:text-[1.563em] font-bold flex w-full text-left ">
                    {state?.price?.toLocaleString('tr-TR', {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 4,
                  ...
                  <div
                    className={`bg-[#f2f2f2] rounded-[8px] w-full h-[36px] ${
                      state?.isSold === true ? 'hidden ' : 'flex'
                    } items-center  overflow-hidden text-ellipsis mr-[10px]`}
                  >
                    <p className="text-[#525252] ml-[10px] text-[0.75em] lg:text-[0.938em] font-normal  w-full text-left ">
                      Verilen Teklif:{' '}
                      <strong className="ml-[4px] text-[0.938em] text-bold">
                        {offer[0]?.offerPrice.toLocaleString('tr-TR', {
                          minimumFractionDigits: 2,
                          maximumFractionDigits: 4,
                        })}{' '}
                        TL
              ...
              <div className="flex flex-row justify-between w-[200px] h-full text-center">
                <div className="flex flex-col ">
                  ...
                  {status >= 0 ? (
                    <div className={'mt-[30px] hidden lg:flex lg:flex-col'}>
                      <p className="text-[#525252] text-[1.563em] font-bold flex w-[200px] text-left ">
                        {state?.price?.toLocaleString('tr-TR', {
                          minimumFractionDigits: 2,
                          maximumFractionDigits: 4,
                      ...
                      <div
                        className={`bg-[#f2f2f2] rounded-[8px] w-[230px] h-[36px] ${
                          state?.isSold === true ? 'hidden ' : 'flex'
                        } items-center max-w-lg overflow-hidden text-ellipsis`}
                      >
                  ...
                  <div
                    className={`${
                      state?.isSold === true
                        ? 'hidden '
                        : 'fixed lg:flex lg:sticky lg:justify-between '
                    }  bottom-10 opacity-[80%] lg:opacity-100   inset-x-0   w-full mt-[30px] h-[45px]`}
                  >
                    <button
                      onClick={() => openBuyModal(true)}
                      className="w-[172px] lg:w-[235px] h-[45px] cursor-pointer bg-[#4B9CE2] text-white rounded-[8px]"
                    >
                      {loading ? <Spinner /> : 'Satın Al'}{' '}
                    </button>
                    {state?.isOfferable ? (
                      <button
                        onClick={() => openModal()}
                        className={` ${
                          status >= 0 ? 'hidden ' : ''
                        } w-[172px] lg:w-[235px] h-[45px] cursor-pointer bg-[#F0F8FF] ml-[10px] text-[#4b9ce2] rounded-[8px] `}
                      >
                        {' '}
                        {offerLoading ? <Spinner /> : 'Teklif Ver'}
                      </button>
                    ) : (
                      <></>
                    )}
                    {status >= 0 ? (
                      <button
                        onClick={() => removeOffer(myOffers[status].id)}
                        className="w-[172px] lg:w-[235px] h-[45px] cursor-pointer bg-[#F0F8FF] ml-[10px] text-[#4b9ce2] rounded-[8px]"
                      >
                        {offerLoading ? <Spinner /> : 'Teklifi Geri Çek'}
                      </button>
                    ) : null}
                  </div>
                  <div className="flex flex-col mt-[15px] text-left flex-wrap w-full mb-[20px] lg:mb-0 ">
                    <h2 className="text-[0.938em] text-[#525252] font-bold">
                      Açıklama
                    </h2>

                    <p className="mt-[10px] text-[0.938em] text-[#555555]">
                      {state?.description}
                    </p>
                  </div>
                </div>
                {showDetailModal || showBuyModal ? (
