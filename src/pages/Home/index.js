...
function Home() {
  ...
  const {allProducts,setActiveCategory,loading,getProducts,activeCategory} = useProduct();
            ...
            !loading ?   
              allProducts?.map(item => (
                <div onClick={()=> handleNavigate(item)} key={item.id} className="flex flex-col  cursor-pointer   w-[173px] h-[266px] md:w-[200px] xl:w-[280px] xl:h-[392px]  xl:px-[10px]   bg-white rounded-[8px]">
                  <img alt={item.id} src={`${item?.image?.url === undefined ?'/images/notfoundimage.jpg': `${process.env.REACT_APP_BASE_ENDPOINT}${item?.image?.url}`}`} onError={({ currentTarget }) => {
              ...
              )):  <div className='h-full w-full justify-center items-center'><Spinner/></div>
          }
